...
import "../../style/catador/loginCatador.css";

import { catadorService } from "../../services/catador/catadorService"; // serviço de login
...
export default function LoginCatador() {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const data = await catadorService.login(formData);

            // salva token e perfil
            localStorage.setItem("token", data.access_token);
            localStorage.setItem("perfil", "catador");

            // navegação SPA
            navigate("/inicioCatador");
        } catch (error) {
            console.error(error);
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="text-center my-3">
                        <a
                            ...
                            onClick={() => navigate("/esqueciMinhasenha-catador")}
                        >
                            Esqueci minha senha
                        </a>
                    </div>
